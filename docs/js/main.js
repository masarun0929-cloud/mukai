...
import { loadAll } from './data.js?v=20260523-artists';
...
import { CHANNELS, COMBINED_CHANNEL, DEFAULT_CHANNEL, SHOW_AUDIENCE_SWITCH, SHOW_COMBINED_CHANNEL, SHOW_SONG_KEYS, SITE } from './config.js?v=20260523-live';
...
function renderHero() {
  ...
  const dataUpdatedAt = stats.updateDate || stats.generatedAt;
  const dSinceUpdate = daysSince(dataUpdatedAt);
  ...
  $('#updated-info').innerHTML =
    chBadge +
    `データ更新日：<strong>${fmtDate(dataUpdatedAt) || '—'}</strong>` +
