...
function resolveSong(raw) {
  ...
  if (aliasTitleMatches.length === 1) {
    return { song: ensureVersionSong(parsed, aliasTitleMatches[0]), parsed };
  }
...
const stats = {
  title: 'むかいせな　歌唱データベース',
  updateText: `データ更新日：${generatedDate.replaceAll('-', '/')}`,
  updateDate: generatedDate,
  generatedAt,
  total: songs.reduce((sum, song) => sum + song.count, 0) || total,
