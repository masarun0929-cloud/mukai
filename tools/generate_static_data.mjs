...
function originalKeywords() {
  return String(process.env.ORIGINAL_GENRE_KEYWORDS || 'むかいせな,せな')
    .split(',')
...
function buildDataset(channel, tables, generatedAt = '') {
  ...
  const generatedDate = generatedAt ? tokyoDateString(generatedAt) : '';
  return {
    stats: {
      title: channel.name,
      updateText: generatedDate ? `データ更新日：${generatedDate.replaceAll('-', '/')}` : '',
      updateDate: generatedDate,
      generatedAt,
      total,
...
function mergeChannels(datasets, generatedAt = '') {
...
async function main() {
  ...
  const channelDatasets = Object.fromEntries(channels.map((channel) => [channel.code, buildDataset(channel, tables, generatedAt)]));
  const combined = mergeChannels(Object.values(channelDatasets), generatedAt);
